...
const conversationsRef = db.ref('private_chats');
...
async function loadUserData() {
    ...
    if (snap.exists()) {
        ...
        currentUserData = { uid: currentUser.uid, username: currentUser.email.split('@')[0], email: currentUser.email, bio: '', avatarUrl: '', followers: {}, following: {}, totalLikes: 0 };
    }
}
...
function renderVideos() {
    ...
    if (filteredVideos.length === 0) {
        container.innerHTML = '<div class="loading"><div class="spinner"></div><span>' + (currentFeed === 'forYou' ? 'لا توجد فيديوهات' : 'تابع مستخدمين لرؤية فيديوهاتهم') + '</span></div>';
        return;
    }
...
async function addNotification(targetUserId, type, fromUserId) {
    ...
    await db.ref(`notifications/${targetUserId}`).push({
        type, fromUserId, fromUsername: fromUser.username,
...
function searchAll() {
    ...
    resultsDiv.innerHTML = `${users.length ? `<div class="mb-5"><h4 class="text-sm opacity-60 mb-2">👥 مستخدمين</h4>${users.map(u => `<div class="search-result" onclick="viewProfile('${u.uid}')"><div class="search-avatar">${u.avatarUrl ? `<img src="${u.avatarUrl}">` : (u.username.charAt(0)?.toUpperCase() || '👤')}</div><div>@${u.username}</div></div>`).join('')}</div>` : ''}${hashtags.length ? `<div class="mb-5"><h4 class="text-sm opacity-60 mb-2"># هاشتاقات</h4>${hashtags.map(h => `<div class="search-result" onclick="searchHashtag('${h.substring(1)}')"><i class="fas fa-hashtag text-[#fe2c55] w-8 text-xl"></i><div>${h}</div></div>`).join('')}</div>` : ''}${videos.length ? `<div><h4 class="text-sm opacity-60 mb-2">🎬 فيديوهات</h4>${videos.map(v => `<div class="search-result" onclick="playVideo('${v.url}')"><i class="fas fa-video w-8 text-xl"></i><div>${(v.description || 'فيديو').substring(0, 40)}</div></div>`).join('')}</div>` : ''}`;
}
...
function searchBySound(soundName) {
    document.getElementById('searchInput').value = soundName;
    closeSounds();
    openSearch();
    searchAll();
}
function loadPopularMusics() {
    const sounds = Object.keys(allSounds).sort((a, b) => allSounds[b] - allSounds[a]);
    popularMusics = sounds.slice(0, 10);
    if (popularMusics.length === 0) popularMusics = ['Original Sound', 'موسيقى هادئة', 'ريمكس ترند', 'أغنية جديدة', 'تيك توك ريمكس'];
}
...
async function loadPrivateMessages(otherUserId) {
    ...
    for (const [msgId, msg] of sortedMessages) {
        ...
        else if (msg.type === 'image') content = `<img src="${msg.imageUrl}" class="message-image" onclick="window.open('${msg.imageUrl}')" style="max-width:200px; max-height:200px; border-radius:16px; cursor:pointer;">`;
        container.innerHTML += `<div class="private-message ${isSent ? 'sent' : 'received'}" style="display:flex; gap:10px; max-width:85%; ${isSent ? 'align-self:flex-end; flex-direction:row-reverse;' : 'align-self:flex-start;'}"><div class="message-content">${content}<div class="message-time" style="font-size:10px; opacity:0.5; margin-top:4px; text-align:${isSent ? 'left' : 'right'}">${time}</div></div></div>`;
    }
...
function resetUploadForm() {
    ...
    document.getElementById('uploadSubmitBtn').classList.remove('disabled');
...
function selectVideoFile(input) {
    const file = input.files[0];
    if (file && file.type.startsWith('video/')) {
        if (file.size > 100 * 1024 * 1024) { alert('حجم الفيديو يجب أن يكون أقل من 100MB'); return; }
        previewVideo(file);
    } else { alert('الرجاء اختيار ملف فيديو صحيح'); }
}
function showHashtagSuggestions() {
    ...
    if (lastWord.startsWith('#')) {
        ...
        if (filtered.length > 0) {
            suggestionsDiv.innerHTML = filtered.map(h => `<span class="hashtag-suggestion" onclick="insertHashtag('${h}')" style="background:rgba(254,44,85,0.2); padding:4px 12px; border-radius:20px; font-size:12px; color:#fe2c55; cursor:pointer;">#${h}</span>`).join('');
...
function showMusicSuggestions() {
    const input = document.getElementById('videoMusic');
    const suggestionsDiv = document.getElementById('musicSuggestions');
    const query = input.value.toLowerCase();
    if (query.length > 0) {
        const filtered = popularMusics.filter(m => m.toLowerCase().includes(query));
        if (filtered.length > 0) {
            suggestionsDiv.innerHTML = filtered.map(m => `<div class="music-suggestion" onclick="selectMusic('${m}')" style="display:flex; align-items:center; gap:8px; background:rgba(254,44,85,0.2); padding:8px 12px; border-radius:20px; margin-bottom:6px; cursor:pointer;"><i class="fas fa-music"></i><div class="music-name">${m}</div><div class="music-count" style="font-size:10px; opacity:0.7;">شائع</div></div>`).join('');
        } else { suggestionsDiv.innerHTML = ''; }
    } else { suggestionsDiv.innerHTML = ''; }
}
function selectMusic(musicName) {
    document.getElementById('videoMusic').value = musicName;
    document.getElementById('musicSuggestions').innerHTML = '';
}
async function uploadVideoWithDetails() {
    ...
    try {
        ...
        await db.ref('videos/').push({
            ...
            description, music, visibility, commentsSetting,
            sender: currentUser.uid,
            senderName: currentUserData?.username,
            likes: 0, likedBy: {}, comments: {},
            timestamp: Date.now()
        });
        statusDiv.innerHTML = '<i class="fas fa-check-circle"></i> تم رفع الفيديو بنجاح!';
        statusDiv.style.color = '#4caf50';
        setTimeout(() => { closeUploadPanel(); renderVideos(); }, 1500);
    } catch (error) {
        console.error('Upload error:', error);
        statusDiv.innerHTML = '<i class="fas fa-exclamation-circle"></i> فشل الرفع: ' + error.message;
...
function switchTab(tab) {
    ...
    if (tab === 'home') {
        closeSearch(); closeNotifications(); closeProfile(); closeSounds(); closeUploadPanel(); closeConversations(); closePrivateChat();
    }
}
