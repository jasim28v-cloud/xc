...
// ========== دوال مساعدة ==========
function showToast(message = '✅ تم نسخ الرابط') {
    ...
    return `
        <div class="admin-panel-section">
            ...
            <div style="margin-bottom:20px"><h4 style="font-weight:bold;margin-bottom:12px">🗑️ حذف فيديوهات</h4><div class="admin-list">${Object.entries(videos).reverse().slice(0, 15).map(([id, v]) => `
                <div class="admin-item"><div class="admin-item-info"><div class="admin-item-avatar"><i class="fas fa-video"></i></div><div class="admin-item-text"><div class="admin-item-name">${(v.description || 'فيديو').substring(0, 35)}</div><div class="admin-item-email">@${v.senderName || 'user'}</div></div></div><button class="admin-delete-btn" onclick="adminDeleteVideo('${id}')">حذف</button></div>
...
async function adminDeleteVideo(videoId) { if (!isAdmin) return; if (confirm('حذف الفيديو؟')) { await db.ref(`videos/${videoId}`).remove(); showToast('✅ تم حذف الفيديو'); setTimeout(() => location.reload(), 500); } }
async function adminBanUser(userId) { if (!isAdmin) return; if (confirm('حظر المستخدم؟')) { await db.ref(`users/${userId}/banned`).set(true); showToast('✅ تم الحظر'); setTimeout(() => location.reload(), 500); } }
async function adminUnbanUser(userId) { if (!isAdmin) return; if (confirm('إلغاء الحظر؟')) { await db.ref(`users/${userId}/banned`).remove(); showToast('✅ تم إلغاء الحظر'); setTimeout(() => location.reload(), 500); } }
async function adminDeleteUser(userId) { if (!isAdmin) return; if (confirm('حذف المستخدم وجميع فيديوهاته؟')) { const videosSnap = await db.ref('videos').once('value'); const videos = videosSnap.val() || {}; for (const [id, v] of Object.entries(videos)) { if (v.sender === userId) await db.ref(`videos/${id}`).remove(); } await db.ref(`users/${userId}`).remove(); showToast('✅ تم حذف المستخدم'); setTimeout(() => location.reload(), 500); } }
...
db.ref('videos').on('value', (s) => {
    const data = s.val();
    if (!data) { allVideos = []; renderVideos(); renderSoundsList(); return; }
    allVideos = []; allSounds = {};
    ...
    filteredVideos.forEach(video => {
        ...
        div.innerHTML = `
            ...
            <div class="video-info">
                ...
                <div class="video-music" onclick="searchBySound('${(video.music || 'Original Sound').replace(/'/g, "\\'")}')"><i class="fas fa-music"></i> ${video.music || 'Original Sound'}</div>
            </div>
...
function initVideoObserver() { 
    ...
    document.querySelectorAll('.video-item').forEach(seg => videoObserver.observe(seg)); 
}
...
async function openNotifications() { 
    ...
    container.innerHTML = ''; 
    for (const [key, n] of Object.entries(notifs).reverse()) { 
        ...
        if (!n.read) await db.ref(`notifications/${currentUser.uid}/${key}/read`).set(true); 
    } 
    panel.classList.add('open'); 
}
...
function renderSoundsList() { 
    ...
    container.innerHTML = sortedSounds.map(([name, count]) => `<div class="sound-item" onclick="searchBySound('${name.replace(/'/g, "\\'")}')"><div class="sound-icon"><i class="fas fa-music"></i></div><div class="sound-info"><div class="sound-name">${name}</div><div class="sound-count">${count} فيديو</div></div></div>`).join(''); 
}
...
function switchTab(tab, ev) {
    ...
    if (ev.target.closest('.nav-item')) ev.target.closest('.nav-item').classList.add('active');
